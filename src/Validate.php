...
namespace Woox\WebmanValidate;

use Closure;
use Woox\WebmanValidate\Exception\ValidateException;
use Woox\WebmanValidate\Helper\Str;
use Webman\File;
use Webman\Http\Request;
use Woox\WebmanValidate\Validate\ValidateRule;
...
class Validate
{
     ...
     * @return $this
     */
    public function remove(array|string $field, mixed $rule = null): Validate
    {
        if (is_array($field)) {
     ...
     * @return $this
     */
    public function append(array|string $field, mixed $rule = null): Validate
    {
        if (is_array($field)) {
            foreach ($field as $key => $rule) {
     ...
     * @return mixed
     */
    public function checkRule(mixed $value, mixed $rules): mixed
    {
     ...
     * @return bool
     */
    public function confirm(mixed $value, mixed $rule, array $data = [], string $field = ''): bool
    {
        if ('' == $rule) {
     ...
     * @return bool
     */
    public function different(mixed $value, mixed $rule, array $data = []): bool
    {
        return $this->getDataValue($data, $rule) != $value;
    }
     ...
     * @return bool
     */
    public function egt(mixed $value, mixed $rule, array $data = []): bool
    {
     ...
     * @return bool
     */
    public function gt(mixed $value, mixed $rule, array $data = []): bool
    {
        return $value > $this->getDataValue($data, $rule);
    }
     ...
     * @return bool
     */
    public function elt(mixed $value, mixed $rule, array $data = []): bool
    {
        return $value <= $this->getDataValue($data, $rule);
    }
     ...
     * @return bool
     */
    public function lt(mixed $value, mixed $rule, array $data = []): bool
    {
        return $value < $this->getDataValue($data, $rule);
    }
     ...
     * @return bool
     */
    public function must(mixed $value): bool
    {
     ...
     * @return bool
     */
    public function ip(mixed $value, string $rule = 'ipv4'): bool
    {
     ...
     * @return bool
     */
    protected function checkExt(File $file, array|string $ext): bool
    {
        if (is_string($ext)) {
            $ext = explode(',', $ext);
        }
     ...
     * @return bool
     */
    protected function checkMime(File $file, array|string $mime): bool
    {
        if (is_string($mime)) {
            $mime = explode(',', $mime);
        }
     ...
     * @return bool
     */
    public function unique(mixed $rule, array $data = [], string $field = ''): bool
    {
        if (is_string($rule)) {
            $rule = explode(',', $rule);
        }
     ...
     * @return bool
     */
    public function requireCallback(mixed $value, mixed $rule, array $data = []): bool
    {
     ...
     * @return bool
     */
    public function requireWithout(mixed $value, mixed $rule, array $data = []): bool
    {
        $val = $this->getDataValue($data, $rule);
     ...
     * @return bool
     */
    public function in(mixed $value, mixed $rule): bool
    {
     ...
     * @return bool
     */
    public function notBetween(mixed $value, mixed $rule): bool
    {
        if (is_string($rule)) {
            $rule = explode(',', $rule);
        }
     ...
     * @return bool
     */
    public function length(mixed $value, mixed $rule): bool
    {
        if (is_array($value)) {
            $length = count($value);
     ...
     * @return bool
     */
    public function max(mixed $value, mixed $rule): bool
    {
        if (is_array($value)) {
            $length = count($value);
        } elseif ($value instanceof File) {
     ...
     * @return bool
     */
    public function min(mixed $value, mixed $rule): bool
    {
        if (is_array($value)) {
            $length = count($value);
        } elseif ($value instanceof File) {
     ...
     * @return bool
     */
    public function after(mixed $value, mixed $rule, array $data = []): bool
    {
     ...
     * @return bool
     */
    public function afterWith(mixed $value, mixed $rule, array $data = []): bool
    {
     ...
     * @return bool
     */
    public function beforeWith(mixed $value, mixed $rule, array $data = []): bool
    {
        $rule = $this->getDataValue($data, $rule);
        return !is_null($rule) && strtotime($value) <= strtotime($rule);
    }
     ...
     * @return bool
     */
    public function allowIp(mixed $value, mixed $rule): bool
    {
        return in_array($value, is_array($rule) ? $rule : explode(',', $rule));
    }
     ...
     * @return bool
     */
    public function denyIp(mixed $value, mixed $rule): bool
    {
        return !in_array($value, is_array($rule) ? $rule : explode(',', $rule));
    }
     ...
     * @return bool
     */
    public function regex(mixed $value, mixed $rule): bool
    {
     ...
     * @access protected
     * @param array $data 数据
     * @param mixed $key 数据标识 支持二维
     * @return mixed
     */
    protected function getDataValue(array $data, mixed $key): mixed
    {
        if (is_numeric($key)) {
            $value = $key;
     ...
     * @return string|array
     */
    protected function getRuleMsg(string $attribute, string $title, string $type, mixed $rule): array|string
    {
     ...
     * @return mixed
     */
    protected function parseErrorMsg(mixed $msg, mixed $rule, string $title): mixed
    {
        if (is_array($msg)) {
            return $this->errorMsgIsArray($msg, $rule, $title);
        }
     ...
     * @return array
     */
    protected function errorMsgIsArray(array $msg, mixed $rule, string $title): array
    {
